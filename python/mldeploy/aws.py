...
def _create_cloudformation_file(name: str) -> NoReturn:
    ...
    if os.path.exists(cf_filename):
        raise FileExistsError(f"A CloudFormation template already exists at the location: {cf_filename}")
    template_version = datetime.now().strftime("%Y%m%d-%H%M%S")
